...
void        _geniePutchar_SoftSerial4D   (uint8_t c, uint32_t baud);
...
uint16_t    _genieGetchar_SoftSerial4D    (void);
...
SoftwareSerial SoftSerial4D(12, 13);
...
uint16_t _genieGetchar_SoftSerial4D (void) {
	...
	if (SoftSerial4D.available() == 0) {
		_genieError = ERROR_NOCHAR;
		return ERROR_NOCHAR;  
	}
	return (uint16_t) SoftSerial4D.read() & 0xFF;
#endif
}
...
///////////////////////////////////////////////////////////////////
// Software Serial port (SoftSerial4D) Tx and init  handler
void _geniePutchar_SoftSerial4D (uint8_t c, uint32_t baud) {
#ifdef SOFTSERIAL4D
    
	if (baud != 0)
		SoftSerial4D.begin (baud);
	else
		SoftSerial4D.write (c);
#endif
}
